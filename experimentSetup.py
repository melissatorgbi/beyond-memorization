...
class runTests_reasoning(runTests_NLI) :

    def __init__( self, data_location, output_location, openai_completion_style, api_path, data_version=None ) :
        
        self.output_location         = output_location
        self.openai_completion_style = openai_completion_style
        self.api_path = api_path

        self.data = self._load_data( data_location )
        self._load_openai_key()
    ...
    def _generate_prompt( self, prompt_number) :

        prompts = CxReasoning_prompts.prompts
        question_bit= """
Option A: {}
Option B: {}
Option C: {} """        
        
        if self.data_version == "target":
            prompt = prompts[ prompt_number] + question_bit
        else:
    ...
    def get_all( self, model, prompt_number, experiment, temperature) :
        ...
        results_by_row = [ [ 'Ex Num', 'Type', 'Text', 'Output', 'Original Text', 'Option A', 'Option B', 'Option C', 'Target', 'Response' ] ]
        for row in tqdm( self.data ) :

            prompt = self._generate_prompt(prompt_number)
            if self.data_version == "target":
                prompt = prompt.format( row[2], row[5], row[6], row[7] )
            else:
                prompt = prompt.format( row[2])
            ...
            results_by_row.append( row )
           #print(results_by_row)
